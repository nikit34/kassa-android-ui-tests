...
class TestProxyMode:
    ...
    def test_002(self, driver):
        self.search_page = SearchPage(driver)
        dbg_api = DebugAPI.run(response=True, mapi_handler=self.search_page.url_creations_performance_schedule_filter)
        try:
            with allure.step('MoviesPage'):
                self.movies_page = MoviesPage(driver)
                self.movies_page.set_custom_wait(20)
    ...
    def test_003(self, driver):
        self.search_page = SearchPage(driver)
        dbg_api = DebugAPI.run(response=True, mapi_handler=self.search_page.url_creations_concert_schedule_filter)
        try:
            with allure.step('MoviesPage'):
                self.movies_page = MoviesPage(driver)
                self.movies_page.set_custom_wait(20)
    ...
    def test_004(self, driver):
        self.search_page = SearchPage(driver)
        dbg_api = DebugAPI.run(response=True, mapi_handler=self.search_page.url_creations_event_schedule_filter)
        try:
            with allure.step('MoviesPage'):
                self.movies_page = MoviesPage(driver)
                self.movies_page.set_custom_wait(20)
